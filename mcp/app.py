...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- NEVER RESEND A CALL THAT SEEMED TO FAIL. A call that does not come back has NOT "
        "failed — it is still running, and resending the code would run it a SECOND time "
        "(doubled writes, doubled installs). Instead call get_execution(kernel_id): it never "
        "touches the kernel, so it answers even while one is busy, and it hands back the "
        "result of an execution your client stopped waiting for. Resend only after CONFIRMING "
        "it never ran.\n"
        "- LONG WORK IS NOT KILLED, AND NOT SILENT. Past the server's reply deadline a "
        "foreground execution returns {status:'still_running', exec_id, partial_output} and "
        "keeps going; collect it with get_execution(kernel_id, wait_seconds=20), which waits "
        "server-side. ONE EXECUTION PER KERNEL: anything else sent meanwhile returns "
        "status='busy' and runs nothing. interrupt_kernel is how you abort on purpose.\n"
