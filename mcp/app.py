...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- ONE EXECUTION PER KERNEL, and NEVER resend a call that seemed to fail. A call that "
        "does not come back has NOT failed — it is still running, and re-sending the code "
        "would run it a SECOND time. While one is in flight the next execute_code/execute_cell "
        "on that kernel returns status='busy' and runs nothing. To find out what happened, call "
        "get_last_execution(kernel_id): it never touches the kernel, so it answers even while "
        "one is busy, and it hands back the result of an execution your client stopped waiting "
        "for.\n"
